...
from panda3d.core import loadPrcFile, Filename, loadPrcFileData, WindowProperties
...
from panda3d.core import PandaNode, NodePath, Mat4
...
class DirectApplication(Application, DirectObject):    
    ...
    def playMusic(self):
        if self.sound:
            self.myMusic = base.loader.loadMusic('%stopo.ogg' % self.soundPath)
    ...
    def createSounds(self):
        ...
        for sound in ['beep01.ogg', 'beep02.ogg', 'beep03.ogg', 'bomb7.ogg', 
            ...
            setattr(self, '%sSound' % sound[:-4], base.loader.loadSfx("%s%s" % (self.soundPath, sound)))        
